...
# Plot
# -----------------------------------------------------------------------------
# Slightly softer/more muted line colors (optional; columns are pastel via fill scale below)
color_pos     <- "#C77C2E"   # softened orange
color_breaks  <- "#B07AA1"   # softened purple
color_markers <- "#C77C2E"   # match positivity tone
color_doses   <- "#6C8EBF"   # softened blue
...
p <- ggplot() +
  ...
  # Stacked columns: reason shares (normalized to sum 100 each month)
  geom_col(
    data = reason_stack,
  ...
  # (3) Segment labels inside when enough room
  geom_text(
    data = reason_stack,
  ...
  # Positivity scaled onto left axis
  geom_line(
    data = df_line,
  ...
  # Doses scaled onto left axis (RIGHT axis shows real doses)
  geom_line(
    data = df_line,
  ...
  # (2) Marker labels moved to the top band (>100, < left_max)
  geom_label(
    data = markers,
  ...
  # (1) Pastel fill for the stacked columns
  scale_fill_brewer(palette = "Pastel2") +

  labs(
    title = "NHS sickness reasons (%) vs Vaccine doses and Test positivity",
    subtitle = "Stacked bars: reason shares (%) | Blue: monthly doses (scaled; see right axis) | Red: test positivity (scaled)",
    x = NULL,
    fill = "Reason (Top 9 + Others)"
  ) +

  coord_cartesian(clip = "off") +  # helps keep top labels visible
...
cat("\nPlot saved as 'nhs_sickness_reason_vs_vax_positivity.png'\n")
